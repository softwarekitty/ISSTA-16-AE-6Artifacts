...
/**
 * represents and reasons about if a feature is present or not
 * 
 * @author cc
 */
public class FeatureSetClass {
	...
	public String diff(FeatureSetClass other) {
		...
		for (int i = 0; i < isFeaturePresentArray.length; i++) {
			if (this.isFeaturePresentArray[i] != other.isFeaturePresentArray[i]) {
				sb.append(": (" + featureDictionary.getName(i) + " - this: " + this.isFeaturePresentArray[i]
						+ ", other: " + other.isFeaturePresentArray[i] + ") :");
			}
		}
		sb.append("]");
		return sb.toString();
	...
	public String toString() {
		return "FeatureSetClass [isFeaturePresentArray=" + Arrays.toString(isFeaturePresentArray) + "]";
	}
}
